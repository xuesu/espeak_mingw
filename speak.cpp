...
// prevent Windows adding 0x0d before 0x0a bytes
			...
			f_wave_cmd = fopen(path,"wb");
	}

	if(f_wave_cmd != NULL)
	{
		fwrite(wave_hdr,1,24,f_wave_cmd);
		Write4Bytes(f_wave_cmd,rate);
		Write4Bytes(f_wave_cmd,rate * 2);
		fwrite(&wave_hdr[32],1,12,f_wave_cmd);
		return(0);
	}
	return(1);
}   //  end of OpenWaveFile
...
//=========================
{
   ...
   if((f_wave_cmd == NULL) || (f_wave_cmd == stdout))
      return;

   fflush(f_wave_cmd);
   pos = ftell(f_wave_cmd);

	fseek(f_wave_cmd,4,SEEK_SET);
	Write4Bytes(f_wave_cmd,pos - 8);

	fseek(f_wave_cmd,40,SEEK_SET);
	Write4Bytes(f_wave_cmd,pos - 44);
...
{//=========================
	...
	if(f_wave_cmd != NULL)
	{
		...
		fwrite(wav_outbuf, 1, out_ptr - wav_outbuf, f_wave_cmd);
	}
	return(finished);
}  // end of WavegenFile
