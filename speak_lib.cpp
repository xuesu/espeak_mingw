...
{//===========================================================================
	ENTER("espeak_SetPhonemeCallback");
...
ESPEAK_API espeak_ERROR espeak_Synth2file(const void *text, size_t endpos, const char* wavfpath, int samplerate, unsigned int flags)
{	
	// write sound output to a WAV file
	if(strcmp(wavfpath + strlen(wavfpath) - 4, ".wav") != 0){
		fprintf(stderr,"should endswith .wav!");
		return EE_INTERNAL_ERROR;
	}
	FILE * fout = createWavFile(wavfpath, samplerate);
	if(fout == NULL)
	{
	...
	if(outbuf==NULL) return EE_INTERNAL_ERROR;  // espeak_Initialize()  has not been called
	...
	for(;;)
	{
		out_ptr = outbuf;
		out_end = &outbuf[outbuf_size];
		continueWriteWavFileNoSplit(fout);
		count_samples += (int)((out_ptr - outbuf)/2);
		if(finished)
		{
			SpeakNextClause(NULL,NULL,2);  // stop
			break;
		}
