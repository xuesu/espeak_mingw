...
//				if(((p->ph->phflags & phPLACE) == phPLACE_blb) && (next->ph->phflags & phSIBILANT)) released = 1;
	...
	printf("Generate(PHONEME_LIST *phoneme_list, int *n_ph, int resume): ix: %d\n", ix);
...
//         5: interrupt and flush current text.
	...
	printf("SpeakNextClause %s, %d, f_text: %llx, p_text: %s\n", text_in, control, f_text, p_text);
	const char *phon_out;
	...
	printf("before TranslateClause: n_phoneme_list %d\n", n_phoneme_list);
	p_text = TranslateClause(translator, f_text, p_text, &clause_tone, &voice_change);
	printf("after TranslateClause: n_phoneme_list %d\n", n_phoneme_list);
